...
//
//  MyRunSubTakeCashViewController.m
//  QuickPos
//
...
@interface MyRunSubTakeCashViewController ()<ResponseData>{
...
@implementation MyRunSubTakeCashViewController
    ...
    if(type == REQUSET_USERINFOQUERY && [dict[@"respCode"]isEqual:@"0000"] ){
       ...
       //用户名的返回
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            agentAmt = [NSString stringWithFormat:@"%0.2f",agent/100];//分润金额
         
         
            self.accountBalanceLabel.text = agentAmt;
            self.CanCashBalancesLabel.text = agentAmt;
        
    }else if([dict[@"respCode"]isEqual:@"0000"] && type == REQUSET_AGENTCASH){
...
//        [request takeCash:turnStr andPassword:self.passwordTextField.text andMobileMac:self.mobileMacTextField.text andCashType:cashType andCardTag:newCardNumber andCardIdx:cardIdx];
         ...
         [request AgentCash:turnStr
                andPassword:self.passwordTextField.text
               andMobileMac:self.mobileMacTextField.text
                andCashType:cashType
                 andCardTag:newCardNumber
                 andCardIdx:cardIdx
          ];
