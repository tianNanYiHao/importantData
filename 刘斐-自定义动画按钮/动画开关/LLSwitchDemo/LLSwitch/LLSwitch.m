...
- (void)initSetUpView {
    ...
    _faceColor = [UIColor whiteColor]; //脸色
    _paddingWidth = self.frame.size.height * 0.1; //脸->按钮框的间隙
    _circleFaceRadius = (self.frame.size.height - 2 * _paddingWidth) / 2; // 脸半径
    _animationDuration = 5.0f;  //动画时间
    _animationManager = [[LLAnimationManager alloc] initWithAnimationDuration:_animationDuration];
    _moveDistance = self.frame.size.width - _paddingWidth * 2 - _circleFaceRadius * 2;  //移动距离
    _on = NO;
    _isAnimating = NO;
...
- (LLEyesLayer *)eyesLayer {
    if (!_eyesLayer) {
        ...
        _eyesLayer.eyeRec = CGRectMake(0, 0, _faceLayerWidth / 6, _circleFaceLayer.frame.size.height * 0.22);
        _eyesLayer.eyeDistance = _faceLayerWidth / 3;
        _eyesLayer.eyeColor = _offColor;
        _eyesLayer.isLiking = NO;
        _eyesLayer.mouthY = _eyesLayer.eyeRec.size.height * 7 / 4;
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if (flag) {
        ...
        if (anim == [_eyesLayer animationForKey:EyesMoveEndAnimationKey]) {
            ...
            if (!_on) {
                CAKeyframeAnimation *eyesKeyFrameAnimation = [_animationManager eyesCloseAndOpenAnimationWithRect:_eyesLayer.eyeRec];
