...
/**
 *  layer background color animation  //颜色渐变动画
 */
...
/**
 * the eyes layer move  //脸部内容Layer部动画
 */
- (CABasicAnimation *)eyeMoveAnimationFromValue:(NSValue *)fromValue toValue:(NSValue *)toValue{
    ...
    moveAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
    moveAnimation.duration = _animationDuration *1/ 3;
    moveAnimation.removedOnCompletion = NO;
    moveAnimation.fillMode = kCAFillModeForwards;
    return moveAnimation;
}
...
/**
 * mouth key frame animation  //嘴部动画
 */
...
/**
 *  eyes close and open key frame animation //眼睁开闭上动画
 */
- (CAKeyframeAnimation *)eyesCloseAndOpenAnimationWithRect:(CGRect)rect {
    ...
    CAKeyframeAnimation *keyAnimation = [CAKeyframeAnimation animationWithKeyPath:@"eyeRec"];
    keyAnimation.values = arrayFrame;
